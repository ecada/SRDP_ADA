...
def train_agent(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    if args.algo == 'ql':
        if args.dp_type == 'ae':
            from agents_ae.ql_diffusion import Diffusion_QL as Agent
            print("ae imported")
        elif args.dp_type == 'base':
        ...
        agent = Agent(state_dim=state_dim,
                      ...
                      grad_norm=args.gn,model_type=args.model_type)
    elif args.algo == 'bc':
        if args.dp_type == 'ae':
            from agents_ae.bc_diffusion import Diffusion_BC as Agent
            print("ae imported")
        elif args.dp_type == 'base':
        ...
        agent = Agent(state_dim=state_dim,
                      ...
                      beta_schedule=args.beta_schedule,
                      n_timesteps=args.T,
                      lr=args.lr,model_type=args.model_type)
    ...
    while (training_iters < max_timesteps) and (not early_stop):
        ...
        if args.dp_type == 'ae':
            logger.record_tabular('BCA Loss', np.mean(loss_metric['bca_loss']))
            logger.record_tabular('BCS Loss', np.mean(loss_metric['bcs_loss']))
        ...
        if args.dp_type == 'ae':
            evaluations.append([eval_res, eval_res_std, eval_norm_res, eval_norm_res_std,
                                ...
                                np.mean(loss_metric['bca_loss']),np.mean(loss_metric['bcs_loss']),
                                curr_epoch])
            wandb.log({"bc_loss": np.mean(loss_metric['bc_loss']), "ql_loss": np.mean(loss_metric['ql_loss']),
                       "actor_loss": np.mean(loss_metric['actor_loss']), "critic_loss": np.mean(loss_metric['critic_loss']),
                       "bca_loss": np.mean(loss_metric['bca_loss']), "bcs_loss": np.mean(loss_metric['bcs_loss']),
                       "eval_res":eval_res,"eval_res_std":eval_res_std, "eval_norm_res":eval_norm_res,"eval_norm_res_std": eval_norm_res_std})
        else:
            evaluations.append([eval_res, eval_res_std, eval_norm_res, eval_norm_res_std,
                                np.mean(loss_metric['bc_loss']), np.mean(loss_metric['ql_loss']),
                                np.mean(loss_metric['actor_loss']), np.mean(loss_metric['critic_loss']),
    ...
    best_res = {'model selection': 'online', 'epoch': scores[best_id, -1],
    ...
    with open(os.path.join(output_dir, f"best_score_online_{args.ms}.txt"), 'w') as f:
        f.write(json.dumps(best_res))
    bc_loss = scores[:, 4]
    top_k = min(len(bc_loss) - 1, args.top_k)
    where_k = np.argsort(bc_loss) == top_k
    best_res_offline = {'model selection':'offline' , 'epochoffline': scores[where_k][0][-1],
                'best normalized score avgoffline': scores[where_k][0][2],
                'best normalized score stdoffline': scores[where_k][0][3],
                'best raw score avgoffline': scores[where_k][0][0],
                'best raw score stdoffline': scores[where_k][0][1]}
    with open(os.path.join(output_dir, f"best_score_offline_{args.ms}.txt"), 'w') as f:
        f.write(json.dumps(best_res_offline))
    if args.dp_type == 'ae':
        bc_loss_a = scores[:, 8]
        top_k_a = min(len(bc_loss_a) - 1, args.top_k)
        where_k_a = np.argsort(bc_loss_a) == top_k_a
        best_res_a_offline = {'model selection': 'offlinea', 'epochoffline': scores[where_k_a][0][-1],
                            'best normalized score avgoffline': scores[where_k_a][0][2],
                            'best normalized score stdoffline': scores[where_k_a][0][3],
                            'best raw score avgoffline': scores[where_k_a][0][0],
                            'best raw score stdoffline': scores[where_k_a][0][1]}
        with open(os.path.join(output_dir, f"best_score_bc_a_offline_{args.ms}.txt"), 'w') as f:
            f.write(json.dumps(best_res_a_offline))

        bc_loss_s = scores[:, 9]
        top_k_s = min(len(bc_loss_s) - 1, args.top_k)
        where_k_s = np.argsort(bc_loss_s) == top_k_s
        best_res_s_offline = {'model selection': 'offlines', 'epochoffline': scores[where_k_s][0][-1],
                            'best normalized score avgoffline': scores[where_k_s][0][2],
                            'best normalized score stdoffline': scores[where_k_s][0][3],
                            'best raw score avgoffline': scores[where_k_s][0][0],
                            'best raw score stdoffline': scores[where_k_s][0][1]}
        with open(os.path.join(output_dir, f"best_score_bc_s_offline_{args.ms}.txt"), 'w') as f:
            f.write(json.dumps(best_res_s_offline))
    # writer.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", default="results_ae", type=str)                    # Logging directory
    ...
    parser.add_argument("--dp_type", default='ae', type=str, help="['base', 'ae']")
    ...
    args.output_dir = f'{args.dp_type}'
    ...
    # Setup Logging
    file_name = f"{args.env_name}|{args.exp}|{args.model_type}|{args.dp_type}|diffusion-{args.algo}|T-{args.T}"
