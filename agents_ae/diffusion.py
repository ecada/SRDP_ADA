...
from agents_ae.helpers import (cosine_beta_schedule,
...
class Diffusion(nn.Module):
    ...
    def p_losses(self, x_start, state, t, weights=1.0):
        ...
        x_recon, s_pred = self.model(x_noisy, t, state)  # noise and state predicted by the ae model
        assert noise.shape == x_recon.shape

        if self.predict_epsilon:
            loss_a = self.loss_fn(x_recon, noise, weights)
        else:
            loss_a = self.loss_fn(x_recon, x_start, weights)
        loss_s = self.loss_fn(s_pred, state, weights)
